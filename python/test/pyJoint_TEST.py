...
class JointTEST(unittest.TestCase):
  ...
  def test_default_construction(self):
    ...
    body = ""
    self.assertEqual(1, len(joint.resolve_child_link(body)))
    self.assertFalse(body)
    self.assertEqual(1, len(joint.resolve_parent_link(body)))
    self.assertFalse(body)
  ...
  def test_copy_construction(self):
    ...
    self.assertEqual("test_joint", joint.name())
    self.assertTrue(None != joint.axis(0))
    self.assertTrue(None != joint.axis(1))
    self.assertEqual(axis.xyz(), joint.axis(0).xyz())
    self.assertEqual(axis1.xyz(), joint.axis(1).xyz())

    self.assertEqual("test_joint", joint2.name())
    self.assertTrue(None != joint2.axis(0))
    self.assertTrue(None != joint2.axis(1))
