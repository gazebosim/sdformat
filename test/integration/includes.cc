...
//////////////////////////////////////////////////
TEST(IncludesTest, Includes_17)
{
  sdf::setFindCallback(findFileCb);

  const auto worldFile =
    sdf::filesystem::append(g_testPath, "sdf", "includes_1.7.sdf");
  ...
  ASSERT_NE(nullptr, root.Element());
  EXPECT_EQ(worldFile, root.Element()->FilePath());
  EXPECT_EQ("1.7", root.Element()->OriginalVersion());

  const sdf::World *world = root.WorldByIndex(0);
  ASSERT_NE(nullptr, world);
  EXPECT_EQ("1.7", world->Element()->OriginalVersion());
  ...
  EXPECT_EQ("F_Z", model->PoseRelativeTo());
  {
    ignition::math::Pose3d pose;
    errors = model->SemanticPose().Resolve(pose);
    EXPECT_TRUE(errors.empty());
    EXPECT_EQ(ignition::math::Pose3d(1, 0, 10, 0, 0, 0), pose);
  }
  EXPECT_EQ(1u, model->LinkCount());
  ASSERT_FALSE(nullptr == model->LinkByIndex(0));
  ASSERT_FALSE(nullptr == model->LinkByName("link"));
