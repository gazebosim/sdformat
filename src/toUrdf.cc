                         ...
                         uint64_t _colIndex)
{
  ...
  stream << PoseToOrigin(_prefix + "  ", col->PoseInFrame(_link->Name()))
         << "\n";
                      ...
                      uint64_t _visIndex)
{
  ...
  stream << PoseToOrigin(_prefix + "  ", vis->PoseInFrame(_link->Name()))
         << std::endl;
    ...
    const sdf::Model *_model)
{
  ...
  ignition::math::Pose3d pose = _joint->PoseInFrame(_joint->ParentLinkName());
