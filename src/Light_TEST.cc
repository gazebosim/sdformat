...
TEST(DOMLight, DefaultConstruction)
{
  ...
  EXPECT_EQ(ignition::math::Pose3d::Zero, light.PoseRaw());
  EXPECT_TRUE(light.PoseRelativeTo().empty());

  light.SetPoseRaw({1, 2, 3, 0, 0, IGN_PI});
  EXPECT_EQ(ignition::math::Pose3d(1, 2, 3, 0, 0, IGN_PI), light.PoseRaw());
...
TEST(DOMLight, CopyConstructor)
{
  ...
  light.SetPoseRaw({3, 2, 1, 0, IGN_PI, 0});
  ...
  EXPECT_EQ(ignition::math::Pose3d(3, 2, 1, 0, IGN_PI, 0), light2.PoseRaw());
