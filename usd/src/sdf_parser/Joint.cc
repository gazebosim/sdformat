...
namespace usd
{
  ...
  /// when setting _joint's pose relative to _joint's parent and child
  sdf::Errors SetUSDJointPose(pxr::UsdPhysicsJoint &_jointPrim,
      const sdf::Joint &_joint, const sdf::Model &_parentModel)
  {
    ...
    else
    {
      ...
      {
        errors.push_back(sdf::Error(sdf::ErrorCode::POSE_RELATIVE_TO_INVALID,
              "Unable to get the pose of joint [" + _joint.Name() +
              "] w.r.t. its parent link [" + _joint.ParentLinkName() + "]."));
        errors.insert(errors.end(), poseResolutionErrors.begin(),
            poseResolutionErrors.end());
        return errors;
      }
    }
  ...
  /// \return Errors, which is a vector of Error objects. Each Error includes
  /// an error code and message. An empty vector indicates no error occurred
  /// when converting _joint to its USD representation.
  sdf::Errors ParseSdfRevoluteJoint(const sdf::Joint &_joint,
      pxr::UsdStageRefPtr &_stage, const std::string &_path)
  {
    ...
    if (!drive)
    {
      // TODO(adlarkin) update this to use USD-specific error code
      // (PR #836 needs to be merged)
      errors.push_back(sdf::Error(sdf::ErrorCode::ELEMENT_INVALID,
  ...
  /// an error code and message. An empty vector indicates no error occurred
  /// when converting _joint to its USD representation.
  sdf::Errors ParseSdfPrismaticJoint(const sdf::Joint &_joint,
      pxr::UsdStageRefPtr &_stage, const std::string &_path)
  {
    sdf::Errors errors;
  ...
  sdf::Errors ParseSdfJoint(const sdf::Joint &_joint,
      pxr::UsdStageRefPtr &_stage, const std::string &_path,
      ...
      const pxr::SdfPath &_worldPath)
  {
    ...
    if (_joint.ParentLinkName() != "world")
    {
      ...
      {
        // TODO(adlarkin) update this error code to be a USD-specific error
        // code once USD-specific error codes are supported
        errors.push_back(sdf::Error(sdf::ErrorCode::ELEMENT_INVALID,
    ...
    switch (_joint.Type())
    {
      ...
      default:
        parsingErrors.push_back(sdf::Error(sdf::ErrorCode::ATTRIBUTE_INVALID,
    ...
    if (!jointPrim)
    {
      // TODO(adlarkin) update this error code to be a USD-specific error
      // code once USD-specific error codes are supported
      errors.push_back(sdf::Error(sdf::ErrorCode::ATTRIBUTE_INVALID,
